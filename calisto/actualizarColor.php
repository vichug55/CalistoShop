<?php
  ...
  if(isset($_SESSION['login'])) {
   ...
   $consulta="SELECT * FROM colores WHERE id='$id'";
   $result=mysqli_query($db,$consulta);
   $colores=mysqli_fetch_assoc($result);

    $color=$colores['color'];
    $imagen=$colores['imagen'];
  ...
  if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    if($imagen['name']){
        ...
        $nombreImagen=$colores['imagen'];
    }

    $query="UPDATE colores SET color='$color', imagen='$nombreImagen' WHERE id='$id'";

    $resultado=mysqli_query($db,$query);


    if($resultado){
        header('Location: /calistoshop/listaColores.php');
    }
        ...
        <fieldset>
            <div class="contenedor-campos--colores">
                ...
                <div class="campo-articulo">
                    ...
                    <input class="input-text" type="color" name="color" value="<?php echo $color ?>" placeholder ="Nombre del articulo">
                </div>
            ...
            <div class="alinear-derecha flex">
                <input class="boton" type="submit" value="Actualizar color" >
            </div>
